...
interface ImportClubModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function ImportClubModal({ isOpen, onClose }: ImportClubModalProps) {
  const { triggerRefresh } = useTournament();
  const [dragActive, setDragActive] = useState(false);
  const [csvContent, setCsvContent] = useState('');
  const [previewRows, setPreviewRows] = useState<ParsedClubRow[]>([]);
  ...
  const [step, setStep] = useState<1 | 2 | 3>(1); // 1: Upload, 2: Preview, 3: Report
  ...
  const downloadCSVTemplate = () => {
    ...
    link.setAttribute("href", `${basePath}/senshi_club_template.csv`);
    link.setAttribute("download", "senshi_club_template.csv");
  ...
  const parseCSV = async (text: string) => {
    try {
      ...
      const existingNames = new Set(existingClubs.map(c => c.name.toLowerCase().trim()));

      const lines = text.split('\n');
      const rows: ParsedClubRow[] = [];
      
      // Skip header line if it contains "Name" or "Club"
      let startIdx = 0;
      if (lines.length > 0) {
        const firstLineLower = lines[0].toLowerCase();
        if (firstLineLower.includes('name') || firstLineLower.includes('dojo') || firstLineLower.includes('city')) {
          startIdx = 1;
        }
      }
      ...
      if (rows.length === 0) {
        alert("No valid club rows parsed. Please check your CSV format.");
        return;
      }
      ...
      <div className="relative w-full max-w-2xl bg-card border border-border rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-muted/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold tracking-tight">Import Dojos & Clubs</h3>
              <p className="text-xs text-muted-foreground">Upload or paste CSV to batch create dojo profiles</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
            ...
            <div className="space-y-4">
              ...
              >
                <input 
                  ...
                  className="hidden" 
                />
                <label htmlFor="club-csv-upload" className="cursor-pointer flex flex-col items-center">
                  ...
                  <p className="text-xs text-muted-foreground mb-3">Format: Name, City, State</p>
              ...
              <textarea
                ...
                placeholder="Paste CSV content here... (e.g. Senshi Karate, Kuala Lumpur, Selangor)"
            ...
            <div className="space-y-4">
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-muted text-muted-foreground font-semibold sticky top-0">
                    <tr>
                      ...
                      <th className="p-2.5 border-b border-border">Dojo / Club Name</th>
                      <th className="p-2.5 border-b border-border">City</th>
                      <th className="p-2.5 border-b border-border">State</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {previewRows.map((row, idx) => (
                      <tr key={idx} className={row.isDuplicate ? 'bg-amber-500/10' : 'hover:bg-muted/20'}>
                        ...
                        <td className="p-2.5 text-muted-foreground">{row.state || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex items-center justify-between pt-3">
                ...
                >
                  ...
                  <span>Import {previewRows.filter(r => !r.isDuplicate).length} New Dojos</span>
                </button>
              </div>
            </div>
          )}
          ...
          {step === 3 && importReport && (
            <div className="space-y-4 text-center py-4">
              ...
              <p className="text-xs text-muted-foreground max-w-md mx-auto">
                Successfully processed dojo roster data.
              </p>
